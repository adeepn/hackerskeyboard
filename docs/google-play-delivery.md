...
20 сентября владелец сообщил результат загрузки alpha04 / `2000004`: Console
блокирует публикацию из-за `targetSdk 26`, требуется минимум 36. Подпись не
является причиной отказа. Это подтверждённый blocker #81; автодоставка #82
ждёт compatibility migration до target 36 и первого принятого выпуска.
Дополнительно Console запрашивает декларацию Advertising ID. В проверенных
исходниках/зависимостях Advertising ID не используется; в декларации выбирается
«Нет», разрешение `AD_ID` не добавляется.

Alpha04 имела `targetSdk 26`. Приоритетный кандидат alpha05 / `2000005` в #95
поднимает target сразу до 36; compile 37 и min 24 сохраняются. Полная voice,
vibration/locale/PackageManager модернизация больше не является предварительным
условием — только выявленные runtime/upload blockers. Общая политика Google с 31 августа 2026 требует API
...
нашего bundle проверена в Console: публикация alpha04 отклонена; результат
сохранён в #81.
Следующая загрузка — только после merge #95, зелёных gates и отдельного запуска
подписи из `v2`. Успешная сборка alpha05 не является подтверждением Play acceptance.
