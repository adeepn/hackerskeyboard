...
До автоматизации Claude review автор запускает локальный CLI и прикладывает к PR
краткое заключение без чувствительных данных. В дальнейшем будет добавлен
репозиторный script с единым prompt и форматом отчёта; секреты Claude не должны
передаваться GitHub-hosted runner без отдельного решения владельца.
