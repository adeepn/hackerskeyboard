...
Каждый PR проходит четыре независимых gate:

1. все локальные и CI-тесты успешны;
2. Codex review финального diff;
3. review финального diff локальной моделью через Claude CLI;
4. review и approval владельца проекта.

Для обоих model reviews используется финальный diff относительно `v2`. Ревью
...
- результат Claude CLI review;
- ссылки на исправления или обоснование отклонённых замечаний;
- локальные команды и CI runs.

После существенного исправления SHA меняется — тесты и оба model reviews
повторяются. Approval старого diff не переносится автоматически.

До автоматизации Claude review владелец запускает CLI вручную и прикладывает к
PR краткое заключение без чувствительных данных. Агенты не запускают Claude CLI,
если в окружении задан `ANTHROPIC_API_KEY`, без отдельного явного разрешения
владельца: наличие ключа означает возможный расход внешнего API budget. В
дальнейшем может быть добавлен репозиторный script с единым prompt и форматом
отчёта; секреты Claude нельзя передавать GitHub-hosted runner без отдельного
решения владельца.
...
- Codex и Claude CLI рассмотрели финальный SHA;
- владелец одобрил PR;
